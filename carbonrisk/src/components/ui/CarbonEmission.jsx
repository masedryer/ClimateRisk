...
const CarbonEmission = ({ selectedCountry, restrictYAxis }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 2) Fetch Carbon Emission data
        const { data: metricData, error } = await supabase
          .from("environmental2")
          .select("Year, carbon_emission")
  ...
  if (data.length === 0) {
    return <p>No Carbon Emission data for {selectedCountry}.</p>;
  }
  ...
  if (restrictYAxis) {
    //
    // LOCKED Y-axis: Carbon Emission from 0.0 to 1.0 in steps of 0.2
    //
    const yAxisSettings = {
      min: 0,
      max: 3000,
