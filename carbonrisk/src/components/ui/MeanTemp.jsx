  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch Mean Temperature data
        const { data: metricData, error } = await supabase
          .from("environment")
          .select("Year, mean_temperature") // Ensure this column exists
          .eq("country_id", countryId);

        if (error) {
          console.error("Error fetching Mean Temperature data:", error.message);
          setLoading(false);
          return;
        }
