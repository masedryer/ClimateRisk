...
const CorruptionIndex = ({ selectedCountry, restrictYAxis }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (error) {
          console.error("Error fetching Corruption Index data:", error.message);
          setLoading(false);
          return;
        }
  ...
  if (data.length === 0) {
    return <p>No Corruption Index data for {selectedCountry}.</p>;
  }
  ...
  if (restrictYAxis) {
    //
    // LOCKED Y-axis: Corruption Index from 0.0 to 1.0 in steps of 0.2
    //
    const yAxisSettings = {
      min: 0,
      max: 1,
    ...
    return (
      <ChartCard
        metricName="Corruption Indext"
        metricData={data}
        countryName={selectedCountry}
        yAxisSettings={yAxisSettings}
        yAxisLabel="Corruption Index (normalised)"
      />
    );
  } else {
    //
